...
const envSchema = z.object({
  ...
  ORCH_TICK_MS: z.coerce.number().int().min(100).default(250),
  ...
  RECONNECT_BASE_DELAY_MS: z.coerce.number().int().min(1000).default(5000),
  RECONNECT_JITTER_MS: z.coerce.number().int().min(0).default(3000),
  MAX_CONCURRENT_SKILLS: z.coerce.number().int().min(1).max(5).default(4),
  ALWAYS_ACTIVE_MODE: z
    .unknown()
    .optional()
    .transform((value) => parseBoolean(value, true)),
  ALWAYS_ACTIVE_REQUEUE_MS: z.coerce.number().int().min(200).default(800),
  ...
  SUBGOAL_RETRY_BASE_DELAY_MS: z.coerce.number().int().min(0).default(300),
  SUBGOAL_RETRY_MAX_DELAY_MS: z.coerce.number().int().min(250).default(2500),
  ...
  PLANNER_COOLDOWN_MS: z.coerce.number().int().min(250).default(2000),
  IDLE_TRIGGER_MS: z.coerce.number().int().min(500).default(3000),
